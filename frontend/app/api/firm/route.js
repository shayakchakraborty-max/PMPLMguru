...
const MAP = { cockpit: "/firm/cockpit", rbac: "/firm/rbac", timesheets: "/firm/timesheets", expenses: "/firm/expenses" };
...
export async function GET(req) {
  ...
  const qs = `owner=${owner}${eid ? `&engagement_id=${encodeURIComponent(eid)}` : ""}`;
  try {
...
export async function POST(req) {
  ...
  const path = action === "expense" ? "/firm/expenses" : action === "expense_status" ? "/firm/expenses/status" : "/firm/timesheets";
  try {
